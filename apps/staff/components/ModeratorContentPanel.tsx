...
import { Archive, BookOpen, ClipboardCheck, Plus, Send, Wrench } from "lucide-react"

type View = "library" | "review" | "archive"
type Status = "draft" | "review" | "approved" | "archived"
type Template = { id: string; title: string; version: string; days: number; exercises: number; status: Status }
...
export function ModeratorContentPanel({ view }: { view: View }) {
  ...
  if (view !== "library") {
    ...
    return <section className="max-w-4xl rounded-3xl border border-line bg-surface p-6"><div className="flex items-center gap-3"><Icon className="h-6 w-6 text-brand" /><div><h2 className="m-0 font-display text-2xl font-medium">{title}</h2><p className="mb-0 mt-1 text-base text-muted">{view === "review" ? "Версии ожидают решения врача. До утверждения их нельзя назначить пациенту." : "Архивные версии сохранены для истории и не изменяются."}</p></div></div><div className="mt-5 grid gap-3">{shown.map((item) => <article key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-line bg-bg p-4"><div><strong>{item.title} {item.version}</strong><p className="mb-0 mt-1 text-sm text-muted">{item.days} дней · {item.exercises} упражнений</p></div>{view === "review" && <button type="button" onClick={() => { setTemplates(templates.map((entry) => entry.id === item.id ? { ...entry, status: "draft" } : entry)); setMessage("Материал возвращён в черновики.") }} className="rounded-xl border border-line px-4 py-2 text-sm font-semibold">Вернуть в черновик</button>}</article>)}</div>{message && <p className="mb-0 mt-5 rounded-2xl bg-tint px-4 py-3 text-sm text-deep">{message}</p>}</section>
  }
...
function ExerciseForm({ onSave }: { onSave: (exercise: Omit<Exercise, "id">) => void }) {
  ...
  return <form onSubmit={(event) => { event.preventDefault(); if (!form.title) return; onSave(form); setForm({ ...form, title: "", direction: "", duration: "" }) }} className="flex flex-col gap-3 rounded-3xl border border-line bg-surface p-6 xl:col-span-5"><div className="flex items-center gap-2"><Wrench className="h-5 w-5 text-brand" /><h2 className="m-0 font-display text-xl font-medium">Новое упражнение</h2></div><Field label="Название" value={form.title} onChange={(title) => setForm({ ...form, title })} required /><Field label="Направление реабилитации" value={form.direction} onChange={(direction) => setForm({ ...form, direction })} /><div className="grid gap-3 sm:grid-cols-2"><Field label="Сложность" value={form.difficulty} onChange={(difficulty) => setForm({ ...form, difficulty })} /><Field label="Длительность" value={form.duration} onChange={(duration) => setForm({ ...form, duration })} /></div><Field label="Языковые версии" value={form.languages} onChange={(languages) => setForm({ ...form, languages })} /><label className="text-sm"><input type="checkbox" checked={form.guardian} onChange={(event) => setForm({ ...form, guardian: event.target.checked })} className="mr-2" />Требуется присутствие опекуна</label><button className="rounded-xl bg-deep px-5 py-3 font-semibold text-white">Сохранить черновик</button></form>
}

function TemplateForm({ exerciseCount, onSave }: { exerciseCount: number; onSave: (template: Omit<Template, "id">) => void }) {
  const [title, setTitle] = useState("")
  const [days, setDays] = useState("14")
  return <form onSubmit={(event) => { event.preventDefault(); if (!title) return; onSave({ title, version: "v1.0", days: Number(days) || 14, exercises: exerciseCount, status: "draft" }); setTitle("") }} className="flex flex-col gap-3 rounded-3xl border border-line bg-surface p-6 xl:col-span-7"><div className="flex items-center gap-2"><Plus className="h-5 w-5 text-brand" /><h2 className="m-0 font-display text-xl font-medium">Конструктор шаблона</h2></div><p className="m-0 text-sm text-muted">Программа → курс → неделя → день → сессия → упражнение. Новый шаблон создаётся черновиком.</p><Field label="Название программы / курса" value={title} onChange={setTitle} required /><Field label="Количество дней" value={days} onChange={setDays} /><div className="rounded-2xl bg-bg p-4 text-sm">В состав войдут упражнения из библиотеки: {exerciseCount}. Дни, сессии и упражнения можно копировать и менять в следующем шаге конструктора.</div><button className="rounded-xl bg-deep px-5 py-3 font-semibold text-white">Создать черновик</button></form>
}
